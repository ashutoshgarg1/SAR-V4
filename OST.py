                    ...
                    if st.session_state.clicked1:
                        ...
                        # File handling logic
                            ...
                            if st.session_state.llm == "Closed-Source":
                                ...
                                prompt = f"You are professional Fraud Analyst. Find answer to the questions as truthfully and in as detailed as possible as per given context only,\n\n\
                                    1. Check if The transaction/disputed amount > 5,000 USD value threshold, If Yes, then what is that transactions/disputed amount ? and also then check below points to make sure if it is a suspicious activity or not: \n\
                                    ...
                                    3. If a suspect is identified from above ,what is the suspect's name?\n\n\
                                    Even if transaction/disputed amount > 5,000 USD but if above criteria does not met, then this can not be considered as a suspicious activity. \n\n\
                                    Analyse above points properly and give your recommendation if this is a case of suspicious activity or not? \n\n\
                                    Context: {contexts}\n\
                                    Response (Give me a concise response in 3 points for each answer with numbering like [1,2])"
                    ...
                    if st.session_state.clicked1:
                        if temp_file_path2 is not None:
                            ...
                            if st.session_state.llm == "Closed-Source":
                                ...
                                Question: {query}\n\
                                Context: {context_1}\n\
                                Response: (Output the identified Products only.)'''
                                response = llama_llm(llama_13b,prompt_1)
