                    ...
                    if st.session_state.clicked1:
                        if temp_file_path2 is not None:
                            ...
                            if st.session_state.llm == "Closed-Source":
                                ...
                                chat_history_1[str(query)] = str(response)
                                ...
                                ques5 = response1
                                chat_history_1[str(query)] = str(response1)
                                ...
                                response = response.replace("$", "USD ")
                                total_cc = response
                                total_cc_firstword = total_cc.split(" ")[0] + total_cc.split(" ")[1] + total_cc.split(" ")[2] + total_cc.split(" ")[3]
                                ...
                                Strictly do not include any Paycheck transactions or Opening balance transaction as they are not be considered as suspicious transactions. Output the "Description", "Date" and "Credited ($)" of those identified transactions. #Also, do not repeat the same transaction.\n\
                                Context: {context_1}\n\
                                Response: (Do not give/add any extra Note, Explanation in answer.) '''
                                #st.write(context_1)
                                response = llama_llm(llama_13b,prompt_1)
                                ...
                                response = response.replace("$", "USD ")
                                total_sav =  response
                                total_sav_firstword = total_sav.split(" ")[0] + total_sav.split(" ")[1] + total_sav.split(" ")[2] + total_sav.split(" ")[3]
                                ...
                                #response = usellm(prompt_1)
                                response = llama_llm(llama_13b,prompt_1)
                                response1 = "Total Money Laundering amount that can be associated with savings account is : USD"+ total_sav_firstword + " and Total Money Laundering amount that can be associated with credit card is : USD"+ total_cc_firstword + " ." + response
                                ques5 = response1
                                chat_history_1[query] = total_sav + total_cc#response1
