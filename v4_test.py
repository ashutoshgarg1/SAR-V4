                    ...
                    if st.session_state.clicked2:
                        ...
                        if st.session_state.llm == "Closed-Source":
                           ...
                           # response_summ_gpt = response_summ_gpt.replace("$", " ")
                            ...
                            text1 = summary2
                            #st.write(text)
                            prompt_1 = f'''Provide a detailed summary of the below Context in one paragraph such that it includes all the relevant information (like names, transactions, involved parties, amounts involved, etc). Don't include words like these: 'chat summary', 'includes information' or 'AI' in the final summary.\n\n\
                            Context: {text1} '''
                            response1 = zephyr_llm(zephyr_7b,prompt_1) 
            ...
            with col1_up:
                ...
                # )
                    ...
                    if st.session_state.clicked:
                        ...
                        #select box to select file
                        fetch=["checking_account_statement.xlsx","credit_card_statement.xlsx","Transaction_Information.pdf","Customer_Relationship_Information.pdf","Blacklisted_Accounts.pdf"]
                    ...
                    if st.session_state.clicked1:
                        if temp_file_path2 is not None:
                            ...
                            if st.session_state.llm == "Closed-Source":
                                ...
                                prompt_1 = f'''Based on the Context, what is the relationship between the suspicious transactions of checking accounts and credit card transactions.\n\n\
                                Context: {context_1}\n\
                               ...
                               # st.image(image, caption='DataFrame as Image', use_column_width=True)
                                ...
                                def run_chain_llm(instruction,query):
                                    ...
                                    {query}
                                    {context}
                                    Answer: (Act as a professional and provide a concise, short response using the context. Do not add anything out of the context provided.) """
                                ...
                                context_1 = docsearch2.similarity_search(query, k=5)
                                prompt_1=f''' Your goal is to identify the suspicious transactions only from Credit_Card_statement within the given Data. Suspicious transactions can be:\n\n
                                Transactions made to a suspicious entity. Output "Description", "Date" and "Debited ($)" of each identified transactions as a numbered list strictly in this format : "Description:  Date:  Debited ($):" . Generate a response without adding a note at the end. # Strictly do not repeat any transaction.\n\
                                Context: {context_1}\n\
                                Response:  (Do not give/add any extra Note, Explanation in answer.)'''
                                ...
                                Output the "Description", "Date" and "Credited ($)" of those identified transactions as a number list of this format : "Description:  Date:  Credited ($):" without repeating the transactions.\n\
                                 Do not repeat the transactions and consider only transactions from checking_account_statement."""
                                query = "What are the associated suspicious transactions for Checking account?"
...
###########
                                ...
                                try:
                                    res_df_llama = pd.DataFrame(list(chat_history_1.items()), columns=['Question','Answer'])
                                ...
                                prompt = f"""Is this a case of Suspicious activity? If yes, then Find answer to the below points as truthfully as possible as per the available information only,\n\n\
                                1.) the transaction triggered\n\
                                2.) total amounts of suspicious transactions for checking account and Credit cards.\n\
                ...
                with st.spinner('Summarization ...'):
                    ...
                    if st.button("Summarize",disabled=st.session_state.disabled):
                        if st.session_state.llm == "Closed-Source":
                            ...
                            context: {text} \n\
                            Response: (Provide the detailed summary in one paragraph such that it includes all the details and numbers and amounts.)'''
                            response1 = zephyr_llm(zephyr_7b,prompt) 
